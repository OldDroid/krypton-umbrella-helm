...
krypton-lib ist ein Helm-**Library-Chart**. Es stellt benannte Templates für Ressourcennamen, Labels, Annotations und Argo-CD-Einstellungen bereit und erzeugt selbst keine Ressourcen. Das verwendende Chart enthält die Templates für Deployments, Container und andere Ressourcen.

## Abhängigkeit einbinden

Liegen `my-app` und `krypton-lib` nebeneinander, wird die Abhängigkeit in `my-app/Chart.yaml` so eingetragen:
...
| Helper mit Präfix `krypton-lib.` | Argumente zusätzlich zu `ctx` | Ergebnis |
| --- | --- | --- |
| `metadata` | `component`; optional `instance`, `shared`, `chart`, `extra`, `annotation`, `annotationsFrom` | Inhalt von `metadata`: Name, Labels und Annotations |
| `componentName` | `component`; optional `instance`, `shared`, `chart` | Ressourcenname |
| `labels` | optional `shared` | Label-Map als YAML |
| `selectorLabels` | keine | Drei Pod-Identitätslabels als YAML-Strings |
| `annotations` | `component`; optional `extra`, `annotation`, `annotationsFrom` | Annotation-Map als YAML |
...
| `image` | optional `image` | Image-Referenz |
| `serviceAccountName` | keine | ServiceAccount-Name |
| `tplValue` | `value` | String als Template ausgewertet; andere Werte als YAML |
...
| `global.syncPrune`, `syncPrune` | Prune-Einstellung je Komponente; lokale Werte haben Vorrang |

Labels werden so zusammengeführt: Standard → `global.labels`. Danach werden die drei reservierten Identitätswerte erneut gesetzt. Eigene Maps können diese Werte somit nicht überschreiben. Für gemeinsame Ressourcen wird `part-of` nach dem Zusammenführen entfernt.

Annotations werden so zusammengeführt: erzeugtes source-chart → `global.annotations` → `extra` → `annotation` → `annotationsFrom` → **nicht leere** Ergebnisse aus `syncWave` und `syncOptions`. Spätere Quellen überschreiben frühere. Alle fertigen Label- und Annotation-Werte werden als Strings ausgegeben. Skalare Werte verwenden; verschachtelte Listen/Maps werden durch die Textumwandlung nicht zu sinnvoller Kubernetes-Metadata. Eigene Schlüssel und Label-Werte werden von der Library nicht vollständig validiert.
...
`syncPrune.<component>: false` erzeugt `Prune=false`; `true` setzt das geerbte Helper-Ergebnis zurück. Booleans und die Strings `"false"`/`"true"` sind vorgesehen. Der Helper vergleicht den Wert als Text mit `"false"`; andere Werte erzeugen keinen Schutz. Deshalb den Typ im verwendenden Chart validieren. Allgemeine Optionslisten bietet nur Slim.
...
## Weitere Helper der großen Library

- `image`: Verwendet ein nicht leeres `image`-Argument, sonst `.Values.image`. `repository` ist Pflicht. `global.imageRegistry` hat Vorrang vor `image.registry`. Ein Digest erzeugt `@digest` und hat Vorrang vor dem Tag. Ohne Digest gilt `image.tag`, danach `Chart.appVersion`; fehlen beide, bricht das Rendern ab. Der Helper prüft nicht die Erreichbarkeit der Registry oder das Digest-Format.
- `serviceAccountName`: Verwendet einen ausdrücklich gesetzten `.Values.serviceAccount.name` direkt. Sonst erzeugt `create: true` einen Namen über `componentName`; bei `create: false` oder fehlendem Schalter wird `default` verwendet. Der Helper erzeugt und prüft keinen ServiceAccount. Explizite Namen werden unverändert übernommen.
- `tplValue`: Wertet Strings mit `tpl` im Kontext `ctx` aus. Andere Werte werden als YAML ausgegeben. Beispielsweise kann ein Vault-Pfad `{{ .Values.global.laneName }}` enthalten. Nur für Values verwenden, deren Template-Auswertung beabsichtigt ist.
...
Unterschied zu Slim: Die große Library bietet zusätzlich `image`, `serviceAccountName` und `tplValue`. Sie verwendet `extra` und `syncPrune`. Subchart-`labels`/`annotations`, `extraLabels`, `extraAnnotations` und allgemeine `syncOptions` werden nur von Slim gelesen. Ein Wechsel ist deshalb mehr als ein Austausch des Helper-Präfixes.
