...
3. `<subchart>.annotations` (krypton-notifier bekommt `krypton.io/on-call`
   aus dem Umbrella)
4. `extraAnnotations` — je Aufruf; die Route reicht `.Values.route.annotations`
   durch (`haproxy.router.openshift.io/timeout`)
5. `annotation` — je Aufruf, ein einzelner `"key=value"`-String für genau diese Ressource
6. `annotationsFrom` — je Aufruf, ein Punkt-Pfad unterhalb von `.Values` auf eine Map
